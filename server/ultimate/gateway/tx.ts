...
import { isLiveTradingEnabled, LIVE_TRADING_DISABLED_REASON } from "../../utils/liveTradingGuard";
...
export async function simulateVtx(
  ...
  try {
    ...
    const value: any = sim.value;
    if (value.err) return { ok: false, err: JSON.stringify(value.err), logs: value.logs || [] };
    return { ok: true, logs: value.logs || [] };
  } catch (e: any) {
    return { ok: false, err: e?.message || "SIM_ERR" };
  }
}

export async function sendVtxWithRetry(
  connection: Connection,
  tx: VersionedTransaction,
  commitment: Commitment = "confirmed" as Commitment
): Promise<{ ok: boolean; sig?: string; err?: string }> {
  if (!isLiveTradingEnabled()) {
    return { ok: false, err: LIVE_TRADING_DISABLED_REASON };
  }
  let attempt = 0;
  let lastErr = "";
