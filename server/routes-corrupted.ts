...
export async function registerRoutes(app: Express): Promise<Server> {
    ...
    try {
      const trades = [
        ...
        },
        {
          ...
          confidence: 89.7,
          timeframe: '3-6 hours',
          signals: ['Jupiter DEX growth', 'DeFi expansion', 'Volume spike'],
          maxLoss: 0.04,
          expectedGain: 0.11
        }
      ];
  ...
  app.get('/api/gpt/insights-performance', (req, res) => {
    try {
      ...
      console.error('GPT performance error:', error);
      res.json({
        success: true,
        gptIntegration: {
          enabled: true,
  ...
  // Configure all services to broadcast via WebSocket
  advancedSellEngine.setWebSocketBroadcast(broadcastToAll);
  aiTradingEngine.setWebSocketBroadcast(broadcastToAll);
